...
SSH_BASE_CMD="ssh -o StrictHostKeyChecking=no -i ~/.ssh/${STREAMING_PROVIDER}_aws"
SCP_BASE_CMD="scp -o StrictHostKeyChecking=no -i ~/.ssh/${STREAMING_PROVIDER}_aws"
...
$SSH_BASE_CMD $SSH_HOST "cd $BENCHMARK_DIR && sudo ./bin/benchmark -d ./driver-${STREAMING_PROVIDER}/deploy/${CLOUD_PROVIDER}/driver.yaml ./workloads/vs/fast-tail-read-500m.yaml"
